...
class QuantumEntanglementAlign(nn.Module):
    ...
    def forward(self, text_feat, image_feat):
        ...
        entanglement = F.cosine_similarity(text_feat, image_feat, dim=-1, eps=1e-6).unsqueeze(-1)
...
class DAMMFNDMODEL(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        # 量子相关模块（与 M2_1tuwenCLIPganshe 保持一致）
    ...
    def forward(self, **kwargs):
        ...
        # clip_fusion_feature = 0.7 * clip_fusion_feature + 0.3 * clip_interference_feat

        # 纠缠对齐后再做融合
        clip_text_feature, clip_image_feature = self.quantum_entanglement_align(clip_text_feature, clip_image_feature)
        clip_fusion_feature = torch.cat((clip_image_feature, clip_text_feature), dim=-1)  # [B,1024]
        clip_fusion_feature = self.clip_fusion(clip_fusion_feature.float())  # [B,320]
