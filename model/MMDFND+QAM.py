...
#对齐用clip
        ...
        # 3. 量子特征投影层 (将BERT/MAE量子特征映射到CLIP维度)
...
#四个融合特征生成最终特征
 ...
 #修改名称后 ========== BERT/MAE特征的量子混合态编码 ==========
        ...
        # bert_mae_interference_intensity: [batch, 768] 干涉强度
        # bert_text_psi/bert_image_psi: 文本/图像量子态波函数
        # bert_quantum_info: 包含振幅、相位、相位差等信息
        ...
        text_feature = text_feature + 0.3 * text_quantum_enhanced  # 残差连接，保留原始信息
        ...
        image_feature = image_feature + 0.3 * image_quantum_enhanced  # 残差连接，保留原始信息        
        # # ========== BERT/MAE特征的量子混合态编码 ==========
        # # 将序列特征编码为量子纯态和混合态
        # text_quantum, image_quantum, bert_mae_fused_quantum, quantum_info = \
        #     self.bert_mae_quantum_encoder(text_feature, image_feature, masks)
        ...
        # ========== 原有量子态编码 ==========
        clip_fusion_feature, image_amplitude, text_amplitude, phase_coherence = \
            self.clip_quantum_fusion(clip_image_feature.float(), clip_text_feature.float())
        # clip_fusion_feature: [64, 320] - 量子态融合特征
        
        # ========== 新增：CLIP量子干涉 ==========
        # 通过量子干涉增强CLIP图文融合
        clip_interference_feat, clip_interference_intensity, clip_image_psi, clip_text_psi, clip_quantum_info = \
            self.clip_quantum_interference(clip_image_feature.float(), clip_text_feature.float())
        # clip_interference_feat: [batch, 320] 干涉融合特征
        
        # ========== 量子特征深度融合 ==========
        # 1. 融合BERT/MAE的量子特征 (混合态 + 干涉态)
        bert_mae_quantum = torch.cat([bert_mae_fused_quantum, bert_mae_interference_feat], dim=-1) # [B, 768*2]
        bert_mae_quantum_proj = self.quantum_proj(bert_mae_quantum) # [B, 320]
        
        # 2. 融合所有量子特征 (CLIP量子态 + CLIP干涉 + BERT/MAE量子投影)
        # 使用残差连接和加权融合，强化量子语义，增大量子特征权重
        clip_fusion_feature = clip_fusion_feature + 0.6 * clip_interference_feat + 0.8 * bert_mae_quantum_proj
        
        # 融合原有特征和干涉特征（加权平均）
        # clip_fusion_feature_final = 0.7 * clip_fusion_feature + 0.3 * clip_interference_feat
...
#2注意力池化
        ...
        fusion_atn_feature = self.MLP_fusion0(fusion_atn_feature)#输出减半
...
#         #     self.clip_quantum_interference(clip_image_feature.float(), clip_text_feature.float())
#         # # clip_interference_feat: [batch, 320] 干涉融合特征
#         # # clip_interference_intensity: [batch, 320] 干涉强度
#         # # clip_image_psi/clip_text_psi: CLIP图像/文本量子态
#         # # clip_quantum_info: 包含振幅、相位、相位相干性等
