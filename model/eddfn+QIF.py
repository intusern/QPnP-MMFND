...
class EDDFNEntangleModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, domain_num, dropout, dataset):
        super(EDDFNEntangleModel, self).__init__()
        if dataset == 'weibo' or dataset == 'ch':
            ...
            raise ValueError(f'Unsupported dataset for EDDFNEntangleModel: {dataset}')
...
class Trainer():
    ...
    def train(self, logger=None):
        ...
        self.model = EDDFNEntangleModel(self.emb_dim, self.mlp_dims, self.domain_num, self.dropout, self.dataset)
        if self.use_cuda:
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                    os.path.join(self.save_param_dir, 'parameter_eddfnentangle.pkl'))
                best_metric = results['metric']
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_eddfnentangle.pkl')))
        results = self.test(self.test_loader)
        if(logger):
            logger.info("start testing......")
            logger.info("test score: {}\n\n".format(results))
        print(results)
        return results, os.path.join(self.save_param_dir, 'parameter_eddfnentangle.pkl')
