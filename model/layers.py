...
class clip_fuion(torch.nn.Module):
    """Clip fusion MLP without BatchNorm to avoid channel-size mismatch at runtime."""
    ...
    def __init__(self, input_dim, out_dim, embed_dims, dropout):
        ...
        for embed_dim in embed_dims:
            layers.append(torch.nn.Linear(input_dim, embed_dim))
            # LayerNorm is shape-safe for (N, C) tensors and avoids BatchNorm running_mean shape errors.
...
class MaskAttention(torch.nn.Module):
    ...
    def forward(self,input,mask):
        # Allow both [B, L, D] and [B, D]; if 2D, treat as single-token sequence
        if input.dim() == 2:
            input = input.unsqueeze(1)

        # score: [B, L]
        score = self.Line(input).squeeze(-1)

        if mask is not None:
            # If mask has extra dims, squeeze; if lengths differ, clip to match
            if mask.dim() > 2:
                mask = mask.squeeze(-1)
            if mask.size(1) != score.size(1):
                mask = mask[:, :score.size(1)]
...
class TokenAttention(torch.nn.Module):
    ...
    def forward(self, inputs):
        # Handle [B, L, D] or [B, D]; if 2D, treat as single token
        if inputs.dim() == 2:
            inputs = inputs.unsqueeze(1)
        elif inputs.dim() != 3:
            raise ValueError(f"TokenAttention expects 2D or 3D input, got shape {inputs.shape}")

        # scores: [B, L]
        scores = self.attention_layer(inputs).squeeze(-1)

        # If only one token, skip softmax to avoid NaNs
        if scores.size(1) == 1:
        ...
        # Return outputs and weights with a singleton head dim for compatibility
