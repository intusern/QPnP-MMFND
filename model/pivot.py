...
class MultiHeadSelfAttention(torch.nn.Module):
    ...
    def dot_score(self, encoder_output):
        # 投影权重按输入精度临时转换，避免半精度输入触发全精度计算
        query = self.dropout(self._linear_cast(encoder_output, self.query))
        key = self.dropout(self._linear_cast(encoder_output, self.key))
        ...
        dots = querys.matmul(keys.permute(0, 1, 3, 2)) / torch.sqrt(torch.tensor(self.head_dim, dtype=querys.dtype))
        #         print(len(dots),dots[0].shape)
        return dots

    def forward(self, encoder_outputs, mask=None):
        # Flatten any leading dims except (seq, hidden) so attention always sees 3D [B, S, H]
        orig_shape = encoder_outputs.shape
        need_reshape = encoder_outputs.dim() > 3
        if need_reshape:
            seq_len, hidden = orig_shape[-2], orig_shape[-1]
            encoder_flat = encoder_outputs.reshape(-1, seq_len, hidden)
        else:
            encoder_flat = encoder_outputs

        # 在注意力内部用半精度以节省显存（输出保持原精度）
        orig_dtype = encoder_flat.dtype
        if encoder_flat.is_cuda and orig_dtype == torch.float32:
            encoder_flat = encoder_flat.half()

        attention_energies = self.dot_score(encoder_flat)
        value = self.dropout(self._linear_cast(encoder_flat, self.value))
        ...
        outputs = self.dropout(self._linear_cast(outputs, self.concat_weight)).to(orig_dtype)

        # Restore original leading dims, if any
        if need_reshape:
            outputs = outputs.reshape(*orig_shape[:-2], orig_shape[-2], orig_shape[-1])
