...
def clipdata2gpu(batch):
    ...
    batch_data = {
        ...
        'category': batch[3].to(device),
        'image':batch[4].to(device),
        'clip_image':batch[5].to(device),
        'clip_text': batch[6].to(device)
    }
    return batch_data
def data2gpu(batch):
    # 根据是否有GPU自动选择设备
...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        try:
            # roc_auc_score返回float，使用round()函数而不是.round()方法
            auc_value = roc_auc_score(res['y_true'], res['y_pred'])
            metrics_by_category[c] = {
    ...
    for c, res in res_by_category.items():
        ...
        y_pred_int = np.around(np.array(res['y_pred'])).astype(int)
        metrics_by_category[c] = {
            'precision': round(precision_score(res['y_true'], y_pred_int, average='macro'), 4),
            'recall': round(recall_score(res['y_true'], y_pred_int, average='macro'), 4),
            'fscore': round(f1_score(res['y_true'], y_pred_int, average='macro'), 4),
            #'auc': metrics_by_category[c]['auc'],
            'acc': round(accuracy_score(res['y_true'], y_pred_int), 4)
        }
    return metrics_by_category
class Recorder():
