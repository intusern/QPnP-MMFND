...
class Run():
    def __init__(self,
        ...
        if config['dataset']=="weibo":
            self.root_path = './data/'
            # 支持 xls 与 csv 两种源文件
            if os.path.exists(self.root_path + 'train_origin.xls'):
                self.train_path = self.root_path + 'train_origin.xls'
                self.val_path = self.root_path + 'val_origin.xls'
                self.test_path = self.root_path + 'test_origin.xls'
            else:
